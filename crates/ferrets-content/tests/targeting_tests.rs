...
//! always declares its layers — the registry refuses an armed type without
//! them — so the only default is on the victim side.

use ferrets_content::{entity_type_def::EntityTypeDef, location::Solidity, targeting};
use ferrets_geometry::cell_size::CellSize;
use ferrets_math::FixedU64;
use ferrets_pathfinder::{layer_mask::LayerMask, nav_grid::LayerId};

const GROUND: LayerId = LayerId::new(1);
const WATER: LayerId = LayerId::new(2);
const AIR: LayerId = LayerId::new(4);
...
fn type_without_weapon_reaches_nothing() {
    ...
    for victim in [walker("grunt", GROUND), walker("ship", WATER), flier()] {
        assert!(
            !targeting::reaches(&bystander, &victim),
...
fn narrowed_weapon_reaches_only_named_layers() {
    let melee = attacker("grunt", GROUND).with_targets(GROUND | WATER);

    assert!(targeting::reaches(&melee, &walker("footman", GROUND)));
    assert!(targeting::reaches(&melee, &walker("ship", WATER)));
    assert!(
        !targeting::reaches(&melee, &flier()),
...
fn weapon_excluding_layer_still_reaches_multi_layer_victim() {
    ...
    let melee = attacker("grunt", GROUND).with_targets(GROUND | WATER);
    let keep = standing("sea_fortress", WATER | AIR);

    assert!(targeting::reaches(&melee, &keep));
}
...
fn declared_targetability_overrides_occupation() {
    ...
    let tower = standing("watch_tower", GROUND).with_targetable(GROUND | AIR);
    let anti_air_only = attacker("interceptor", AIR).with_targets(AIR);
    let melee = attacker("grunt", GROUND).with_targets(GROUND);

    assert_eq!(targeting::targetable(&tower), GROUND | AIR);
    assert!(
        targeting::reaches(&anti_air_only, &tower),
        "an air-only weapon must reach a tower that declares itself an air target"
    );
    assert!(
        targeting::reaches(&melee, &tower),
...
fn declared_targetability_can_exclude_own_ground() {
    ...
    let totem = standing("spirit_totem", GROUND).with_targetable(AIR);
    let melee = attacker("grunt", GROUND).with_targets(GROUND);
    let anti_air = attacker("interceptor", AIR).with_targets(AIR);

    assert_eq!(targeting::targetable(&totem), LayerMask::from(AIR));
    assert!(
        !targeting::reaches(&melee, &totem),
        "a ground weapon reached a victim that declared itself away from the ground"
    );
    assert!(targeting::reaches(&anti_air, &totem));
}
...
fn empty_targets_panics() {
    attacker("grunt", GROUND).with_targets(LayerMask::EMPTY);
}

#[test]
#[should_panic(expected = "entity type 'watch_tower' is targetable on no layers")]
fn empty_targetable_panics() {
    standing("watch_tower", GROUND).with_targetable(LayerMask::EMPTY);
}
...
/// A type that stands still on `occupation`.
fn standing(name: &str, occupation: impl Into<LayerMask>) -> EntityTypeDef {
    EntityTypeDef::new(name).with_location(occupation, CellSize::ONE, Solidity::Solid)
}
...
fn attacker(name: &str, occupation: LayerId) -> EntityTypeDef {
    walker(name, occupation).with_attack(5, 1, 5, 10, 5)
}
