...
use crate::{cell_pos::CellPos, cell_rect::CellRect};
...
impl Projection {
    ...
    pub fn in_range(self, from: CellPos, to: CellPos, distance: u32) -> bool {
        match self {
            Projection::Isometric => chebyshev(from, to) <= distance,
            Projection::Orthogonal => {
                let dx = from.x.abs_diff(to.x) as u64;
                let dy = from.y.abs_diff(to.y) as u64;
    ...
    /// the same metric as [`in_range`](Self::in_range).
