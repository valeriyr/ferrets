...
fn shot_lands_after_its_flight_time() {
    let mut app = app();
    let (_, gunner) = spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 5), Some(0))
        .expect("gunner spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(9, 5), Some(1))
...
fn blast_damages_bystanders_by_band() {
    let mut app = app();
    let (_, gunner) = spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 20), Some(0))
        .expect("gunner spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(9, 20), Some(1))
            .expect("target spawns");
    // One cell from the impact (half damage) and two cells away (quarter damage).
    let (near, _) = spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(10, 20), Some(1))
        .expect("near bystander spawns");
    let (far, _) = spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(11, 20), Some(1))
...
fn blast_spares_own_side_without_friendly_fire() {
    let mut app = app();
    let (_, gunner) = spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 26), Some(0))
        .expect("gunner spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(9, 26), Some(1))
            .expect("target spawns");
    // An own-side unit standing inside the blast.
    let (ally, _) = spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(10, 26), Some(0))
        .expect("ally spawns");
...
fn blast_scales_bonus_and_subtracts_armor_in_full() {
    ...
    let mut app = app();
    let (_, gunner) = spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 30), Some(0))
        .expect("gunner spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(9, 30), Some(1))
            .expect("target spawns");
    let (tank, _) = spawn::spawn_entity(app.world_mut(), "tank", utils::pos(10, 30), Some(1))
...
fn cell_aimed_shot_misses_target_that_moves_away() {
    ...
    let mut app = app();
    let (_, sieger) = spawn::spawn_entity(app.world_mut(), "sieger", utils::pos(5, 4), Some(0))
        .expect("sieger spawns");
    let (runner, runner_id) =
        spawn::spawn_entity(app.world_mut(), "runner", utils::pos(11, 4), Some(1))
...
fn cell_aimed_shot_hits_whoever_stands_on_cell() {
    ...
    let mut app = app();
    let (_, sieger) = spawn::spawn_entity(app.world_mut(), "sieger", utils::pos(5, 8), Some(0))
        .expect("sieger spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(11, 8), Some(1))
            .expect("target spawns");
...
fn cell_aiming_weapon_can_be_ordered_onto_bare_ground() {
    ...
    let mut app = app();
    let (_, sieger) = spawn::spawn_entity(app.world_mut(), "sieger", utils::pos(5, 14), Some(0))
        .expect("sieger spawns");
...
fn target_following_weapon_refuses_ground_order() {
    ...
    let (gunner_entity, gunner) =
        spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 18), Some(0))
            .expect("gunner spawns");
...
fn shot_lands_after_its_attacker_dies() {
    let mut app = app();
    let (gunner_entity, gunner) =
        spawn::spawn_entity(app.world_mut(), "gunner", utils::pos(5, 12), Some(0))
            .expect("gunner spawns");
    let (target, target_id) =
        spawn::spawn_entity(app.world_mut(), "dummy", utils::pos(9, 12), Some(1))
            .expect("target spawns");
