...
fn attack_kills_adjacent_target() {
    ...
    let (target, target_id) = utils::create_entity(world, "dummy", utils::pos(6, 5), None).unwrap();
...
fn wide_attacker_stops_at_its_edge_reach() {
    ...
    let (ballista, ballista_id) =
        utils::create_entity(world, "ballista", utils::pos(2, 5), Some(0)).unwrap();
    let (target, target_id) =
        utils::create_entity(world, "dummy", utils::pos(10, 5), None).unwrap();
...
fn send_to_entity_does_not_attack_ally() {
    ...
    let world = app.world_mut();
    let (_, actor_id) = utils::create_entity(world, "soldier", utils::pos(5, 5), Some(0)).unwrap();
    let (ally, ally_id) =
...
fn attack_gives_up_on_walled_in_target() {
    ...
    let (_, attacker_id) =
        utils::create_entity(app.world_mut(), "soldier", utils::pos(5, 5), Some(0)).unwrap();
    let (target, target_id) =
        utils::create_entity(app.world_mut(), "dummy", utils::pos(15, 15), None).unwrap();
    for (x, y) in [
        (14, 14),
        (15, 14),
        (16, 14),
        ...
        utils::create_entity(app.world_mut(), "dummy", utils::pos(x, y), None)
...
fn turret_holds_fire_until_it_bears_on_target() {
    ...
    let (gun, gun_id) = utils::create_entity(world, "bastion", utils::pos(5, 8), Some(0)).unwrap();
    let (target, target_id) = utils::create_entity(world, "dummy", utils::pos(5, 3), None).unwrap();
    let square = app
        .world()
...
fn turret_tracks_target_beyond_reach_without_walking() {
    ...
    let (gun, gun_id) = utils::create_entity(world, "bastion", utils::pos(5, 15), Some(0)).unwrap();
    let (target, target_id) = utils::create_entity(world, "dummy", utils::pos(5, 5), None).unwrap();
...
fn walking_gun_keeps_hull_heading_while_it_aims() {
    ...
    let world = app.world_mut();
    let (wagon, wagon_id) =
        utils::create_entity(world, "gun_wagon", utils::pos(5, 5), Some(0)).unwrap();
    let (_, target_id) = utils::create_entity(world, "dummy", utils::pos(9, 1), None).unwrap();
...
fn gun_keeps_its_bearing_after_its_fight_ends() {
    ...
    let world = app.world_mut();
    let (gun, gun_id) = utils::create_entity(world, "bastion", utils::pos(5, 8), Some(0)).unwrap();
    let (_, target_id) = utils::create_entity(world, "dummy", utils::pos(5, 3), None).unwrap();
    let mounted = utils::bearing_of(app.world(), gun);
...
fn rolling_gun_shoots_what_it_drives_past() {
    ...
    let (target, _) = utils::create_entity(world, "dummy", utils::pos(11, 7), Some(1)).unwrap();
...
fn rolling_gun_tracks_what_it_cannot_yet_reach() {
    ...
    let (target, _) = utils::create_entity(world, "dummy", utils::pos(11, 4), Some(1)).unwrap();
...
fn hull_keeps_its_heading_while_gun_fights() {
    ...
    let (wagon, wagon_id) =
        utils::create_entity(world, "rolling_gun", utils::pos(5, 10), Some(0)).unwrap();
    utils::create_entity(world, "dummy", utils::pos(11, 7), Some(1)).unwrap();
...
fn ordered_gun_works_target_while_it_closes() {
    ...
    let (target, target_id) =
        utils::create_entity(world, "dummy", utils::pos(13, 10), Some(1)).unwrap();
...
fn ordered_gun_keeps_to_target_it_was_given() {
    ...
    let (bystander, _) = utils::create_entity(world, "dummy", utils::pos(9, 8), Some(1)).unwrap();
...
fn held_fire_works_target_it_was_ordered_onto() {
    ...
    let (_, wagon_id) =
        utils::create_entity(world, "rolling_gun", utils::pos(5, 10), Some(0)).unwrap();
    let (target, target_id) =
        utils::create_entity(world, "dummy", utils::pos(5, 8), Some(1)).unwrap();
...
fn ground_attack_takes_gun_back_from_its_last_target() {
    ...
    let (_, mortar_id) =
        utils::create_entity(world, "rolling_mortar", utils::pos(5, 10), Some(0)).unwrap();
    let (held, held_id) = utils::create_entity(world, "dummy", utils::pos(5, 8), Some(1)).unwrap();
    let (shelled, _) = utils::create_entity(world, "dummy", utils::pos(9, 10), Some(1)).unwrap();
...
fn halting_gun_drives_past_without_shooting() {
    ...
    let world = app.world_mut();
    let (_, wagon_id) =
        utils::create_entity(world, "gun_wagon", utils::pos(5, 10), Some(0)).unwrap();
    let (target, _) = utils::create_entity(world, "dummy", utils::pos(11, 7), Some(1)).unwrap();
...
fn halting_gun_fires_where_it_stands() {
    ...
    let (_, wagon_id) =
        utils::create_entity(world, "gun_wagon", utils::pos(5, 10), Some(0)).unwrap();
    let (target, target_id) =
        utils::create_entity(world, "dummy", utils::pos(8, 10), Some(1)).unwrap();
...
fn ordered_attack_fires_one_cycle_only() {
    ...
    let (target, target_id) =
        utils::create_entity(world, "dummy", utils::pos(5, 3), Some(1)).unwrap();
...
fn idle_gun_hunts_what_it_cannot_reach_without_walking() {
    ...
    let (target, _) = utils::create_entity(world, "dummy", utils::pos(5, 4), Some(1)).unwrap();
...
fn held_fire_keeps_idle_guns_still() {
    ...
    let (wagon, wagon_id) =
        utils::create_entity(world, "rolling_gun", utils::pos(5, 10), Some(0)).unwrap();
    let (target, _) = utils::create_entity(world, "dummy", utils::pos(5, 7), Some(1)).unwrap();
