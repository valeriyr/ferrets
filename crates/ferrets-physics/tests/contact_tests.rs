...
fn heavier_resting_body_yields_less_than_lighter() {
    let pushes = contact::separations(&[
        utils::weighing(utils::resting(2.0, 5.0), 1.0),
        utils::weighing(utils::resting(2.5, 5.0), 3.0),
    ]);
    ...
    assert_eq!(
        pushes,
        vec![utils::push(-0.375, 0.0), utils::push(0.125, 0.0)]
    );
}
...
fn walker_pressing_far_heavier_rester_takes_larger_share() {
    let pushes = contact::separations(&[
        utils::weighing(utils::walking(2.0, 5.0, 1.0, 0.0), 1.0),
        utils::weighing(utils::resting(2.5, 5.0), 9.0),
    ]);

    assert_eq!(
        pushes,
        vec![utils::push(-0.375, 0.09375), utils::push(0.125, -0.03125)],
...
fn heavy_walker_plows_light_rester_aside() {
    let pushes = contact::separations(&[
        utils::weighing(utils::walking(2.0, 5.0, 1.0, 0.0), 5.0),
        utils::resting(2.5, 5.0),
    ]);
...
fn mutual_press_splits_by_weight_alone() {
    let pushes = contact::separations(&[
        utils::weighing(utils::walking(2.0, 5.0, 1.0, 0.0), 1.0),
        utils::weighing(utils::walking(2.5, 5.0, -1.0, 0.0), 3.0),
    ]);
...
fn truncated_weight_split_parts_by_full_separation() {
    ...
    let pushes = contact::separations(&[
        utils::weighing(utils::resting(2.0, 5.0), 1.0),
        utils::weighing(utils::resting(2.5, 5.0), 2.0),
    ]);
...
fn equal_weights_part_symmetrically_whichever_order() {
    let pushes = contact::separations(&[utils::resting(2.0, 5.0), utils::resting(2.5, 5.0)]);
    let reversed = contact::separations(&[utils::resting(2.5, 5.0), utils::resting(2.0, 5.0)]);
...
fn weightless_body_yields_whole_separation() {
    let pushes = contact::separations(&[
        utils::weighing(utils::resting(2.0, 5.0), 0.0),
        utils::weighing(utils::resting(2.5, 5.0), 1.0),
    ]);

    assert_eq!(pushes, vec![utils::push(-0.5, 0.0), utils::push(0.0, 0.0)]);
}
...
fn weightless_bodies_part_evenly() {
    let pushes = contact::separations(&[
        utils::weighing(utils::resting(2.0, 5.0), 0.0),
        utils::weighing(utils::resting(2.5, 5.0), 0.0),
    ]);
...
fn weight_past_stiffening_range_saturates_rather_than_wrapping() {
    let against_parked = |weight: f64| {
        contact::separations(&[
            utils::weighing(utils::walking(2.0, 5.0, 1.0, 0.0), weight),
            utils::resting(2.5, 5.0),
        ])
    };
    ...
    let saturated = against_parked(715_827_883.0);
    assert_eq!(saturated, against_parked(4_000_000_000.0));
...
fn weightless_walker_yields_whole_separation() {
    let pushes = contact::separations(&[
        utils::weighing(utils::walking(2.0, 5.0, 1.0, 0.0), 0.0),
        utils::resting(2.5, 5.0),
    ]);

    assert_eq!(
        pushes,
        vec![utils::push(-0.5, 0.125), utils::push(0.0, 0.0)],
