...
fn taking_off_swaps_occupied_layer() {
    ...
    let (gryphon, gryphon_id) = spawn::spawn_entity(
        app.world_mut(),
        "gryphon",
...
fn changing_form_is_refused_when_riders_would_not_fit() {
    ...
    for offset in [3, 4] {
        let (_, rider) = spawn::spawn_entity(
            app.world_mut(),
            "archer",
...
fn ordered_change_takes_forms_morph_time() {
    let mut app = utils::demo_map_app(MovementModel::Continuous);
    let (gryphon, _) = spawn::spawn_entity(
        app.world_mut(),
        "gryphon",
        utils::at_cell(CLEAR.0, CLEAR.1),
        Some(0),
...
fn changed_gryphon_flies_over_what_stopped_it_on_foot() {
    ...
    let (gryphon, _) =
        spawn::spawn_entity(app.world_mut(), "gryphon", utils::at_cell(44, 12), Some(0))
...
fn rider_fires_from_moving_holder() {
    ...
    .expect("the gryphon spawns");
    let (rider, rider_id) = spawn::spawn_entity(
        app.world_mut(),
        "archer",
    ...
    let (_, victim_id) = spawn::spawn_entity(
        app.world_mut(),
        "grunt",
...
fn death_flushes_change_under_way() {
    let mut app = utils::demo_map_app(MovementModel::Continuous);
    let (gryphon, id) = spawn::spawn_entity(
        app.world_mut(),
        "gryphon",
        utils::at_cell(CLEAR.0, CLEAR.1),
        Some(0),
...
fn morph_command_gates_on_declared_transitions() {
    ...
    let (peasant, peasant_id) = spawn::spawn_entity(
        app.world_mut(),
        "peasant",
        utils::at_cell(CLEAR.0, CLEAR.1),
        Some(0),
...
fn upgrade_pays_up_front_and_lands() {
    let mut app = utils::demo_map_app(MovementModel::Continuous);
    let (tower, _) = spawn::spawn_entity(
        app.world_mut(),
        "watch_tower",
        utils::at_cell(CLEAR.0, CLEAR.1),
        Some(1),
